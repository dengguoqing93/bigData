...
public class QuorumPeerConfig {
    ...
    public void parse(String path) throws ConfigException {
        ...
        try {
            ...
            // TODO_MA 注释： 启动的时候，如果发现 zoo.cfg 不存在，则报错
            if(!configFile.exists()) {
                throw new IllegalArgumentException(configFile.toString() + " file is missing");
            }

            // TODO_MA 注释： 读取 zoo.cfg 文件成为一个 Properties 对象
            ...
            // TODO_MA 注释： 执行解析
            // TODO_MA 注释： 原来的参数在配置文件zoo.cfg 中，现在要解析到 QuorumPeerConfig 类中。
            // TODO_MA 注释： QuorumPeerConfig 存在于一个zookeeper 节点中，用来管理 zookeeper 的参数
            parseProperties(cfg);
    ...
    public void parseProperties(Properties zkProp) throws IOException, ConfigException {
        ...
        for(Entry<Object, Object> entry : zkProp.entrySet()) {
            ...
            if(key.equals("dataDir")) {
                ...
                // TODO_MA 注释： electionAlg 用来配置选举算法的， 默认等于3： 选举算法是： FastLeaderElection
                ...
                // TODO_MA 注释： 解析 当前节点的 角色
                // TODO_MA 注释： peerType 有两种： observer + participant(有选举权和被选举权)
                ...
                // TODO_MA 注释： value 值的长度，必须要么是2， 要么是3， 要么是4
                if((parts.length != 2) && (parts.length != 3) && (parts.length != 4)) {
                    LOG.error(value + " does not have the form host:port or host:port:port " + " or host:port:port:type");
                }
                LearnerType type = null;

                // TODO_MA 注释： parts[0] 主机名称
                String hostname = parts[0];

                // TODO_MA 注释： parts[1] 心跳端口
                Integer port = Integer.parseInt(parts[1]);
                Integer electionPort = null;
                if(parts.length > 2) {

                    // TODO_MA 注释： parts[2] 选举端口
                    electionPort = Integer.parseInt(parts[2]);
                }
                if(parts.length > 3) {

                    // TODO_MA 注释： parts[3] (observer  或者  participant)
                    // TODO_MA 注释： participant 不能写成 leader, follower, learner, ... 默认不用写
        ...
        if(servers.size() == 0) {
             ...
             *  注释： 解析 myid
             *  有三个类似的概念：  serverid  myid  sid  都是一个概念！
             *  安装 zk 集群的时候，都会为每一个节点指定一个不一样的  myid
             */
            ...
            if(roleByServersList != peerType) {
                LOG.warn(
                        "Peer type from servers list (" + roleByServersList + ") doesn't match peerType (" + peerType + "). Defaulting to servers list.");
