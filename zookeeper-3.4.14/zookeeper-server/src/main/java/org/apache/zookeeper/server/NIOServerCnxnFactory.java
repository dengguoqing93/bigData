...
public class NIOServerCnxnFactory extends ServerCnxnFactory implements Runnable {
    ...
    */
    ...
    public void configure(InetSocketAddress addr, int maxcc) throws IOException {
        ...
        // TODO_MA 注释： Factory 内部有一个成员变量 thread
        // TODO_MA 注释： 将来会调用 factory.start()
        // TODO_MA 注释： 其实内部就是调用 thread.start()
        // TODO_MA 注释： 最终跳转到 factory 的 run() 方法
         ...
         *  注释： 用一句话描述，启动了 NIO 的服务端，并且绑定了 2181 端口，等待 客户端发送请求过来
         */
        this.ss = ServerSocketChannel.open();
        ss.socket().setReuseAddress(true);
        LOG.info("binding to port " + addr);   // addr = 2181
        ss.socket().bind(addr);
