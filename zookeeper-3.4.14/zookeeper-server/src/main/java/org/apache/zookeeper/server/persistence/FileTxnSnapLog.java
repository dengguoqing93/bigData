...
public class FileTxnSnapLog {
     ...
     *  注释： FileTxnSnapLog 负责数据的处理
     *  1、TxnLog： 负责日志相关（写请求处理过程中，负责记录操作日志）
     *  2、SnapShot： 磁盘数据文件 和 内存数据 之间的相互转化
     */
    ...
    public long restore(DataTree dt, Map<Long, Integer> sessions, PlayBackListener listener) throws IOException {
        ...
        // TODO_MA 注释： 把磁盘数据(快照数据)恢复到内存
        snapLog.deserialize(dt, sessions);

        // TODO_MA 注释： 从操作日志中恢复数据
        // TODO_MA 注释： 方法返回 当前 zookeeper 节点的最大 zxid
        ...
        try {
            ...
            while(true) {
                ...
                try {
                     ...
                     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                     *  注释： 事务 replay
                     *  每一次事务操作，都有日志，而且 对应的事务 ID 是越来越大的。
                     */
        ...
        switch(hdr.getType()) {
            case OpCode.createSession:
                ...
                if(LOG.isTraceEnabled()) {
                    ZooTrace.logTraceMessage(LOG, ZooTrace.SESSION_TRACE_MASK, "playLog --- create session in log: 0x" + Long
                            .toHexString(hdr.getClientId()) + " with timeout: " + ((CreateSessionTxn) txn).getTimeOut());
                }
