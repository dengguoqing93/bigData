...
public abstract class ServerCnxnFactory {
    ...
    static public ServerCnxnFactory createFactory() throws IOException {
        ...
        // TODO_MA 注释： 获取 工厂实现类 是谁
        String serverCnxnFactoryName = System.getProperty(ZOOKEEPER_SERVER_CNXN_FACTORY);
        if(serverCnxnFactoryName == null) {

            // TODO_MA 注释： NIOServerCnxnFactory 就是默认的 服务端的通信工厂类的实现
            serverCnxnFactoryName = NIOServerCnxnFactory.class.getName();
        }

        // TODO_MA 注释： 通过反射创建这个类的实例对象
        try {
    ...
    protected void configureSaslLogin() throws IOException {
        ...
        try {
            ...
            throw new IOException(
                    "Could not configure server because SASL configuration did not allow the " + " ZooKeeper server to authenticate itself properly: " + e);
        }
    }
}
