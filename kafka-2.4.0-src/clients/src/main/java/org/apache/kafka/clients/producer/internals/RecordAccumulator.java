...
public final class RecordAccumulator {
            ...
            synchronized (deque) {
                ...
                if (batch != null) {
                    ...
                    if (leader == null) {
                        ...
                        //重试次数是否大于0，且等待时间小于重试的间隔时间
                        boolean backingOff = batch
                                .attempts() > 0 && waitedTimeMs < retryBackoffMs;
                        /**
                         * 最多等待的时间
                         * 指定时间内没有写满Batch,也要发送数据出去
                         * linerMs:batch是否写满，都要发送该批次数据，该值默认值为0。在使用过程中需要修改该值
                         * retryBackOffMs:重试时间间隔
                         */
                        long timeToWaitMs =
                                backingOff ? retryBackoffMs : lingerMs;
                        /**
                         * batch是否已经写满
                         * 1、如果deque里面的批次大于1,说明至少有一个batch写满了
                         * 2、说明当前的batch刚好写满了
                         */
                         ...
                         * full:有批次已经写满了
                         * expired:时间已经到了
                         * exhausted:说明内存已经不足了，需要发送消息
                         * close||flushInProgress，在Producer
                         * 关闭的时候，需要把内存的数据发送出去，避免消息丢失
                         */
                        boolean sendable = full || expired || exhausted || closed || flushInProgress();
                        //可以发送消息
                        if (sendable && !backingOff) {
                            //把可以发送【批次】的partition的leader
                            // partition所在的主机加入到readyNodes
                            readyNodes.add(leader);
                        } else {
                            //
                            long timeLeftMs = Math
                                    .max(timeToWaitMs - waitedTimeMs, 0);
                            // Note that this results in a conservative estimate since an un-sendable partition may have
                            // a leader that will later be found to have sendable data. However, this is good enough
                            // since we'll just wake up and then sleep again for the remaining time.
                            nextReadyCheckDelayMs = Math
                                    .min(timeLeftMs, nextReadyCheckDelayMs);
                        }
                    }
                }
            }
    ...
    private Deque<ProducerBatch> getOrCreateDeque(TopicPartition tp) {
        ...
        if (d != null)
            return d;
        d = new ArrayDeque<>();
        //这里这个batches是一个读写分离的Map。有一个写操作的锁
